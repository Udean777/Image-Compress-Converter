<script lang="ts">
	...
	import { IconCloud } from '$lib/components/icons';
	import { Database, Trash2, Plus } from '@lucide/svelte';
	...
	$effect(() => {
		const status = $page.url.searchParams.get('connect');
		if (status === 'success') {
...
<div class="space-y-8">
	...
	<div class="relative min-h-125">
		{#if !data.isPro}
			<div
				class="absolute inset-0 z-50 flex flex-col items-center justify-center rounded-xl bg-background/80 backdrop-blur-sm"
			>
				<div class="max-w-md space-y-4 rounded-lg border bg-background p-6 text-center shadow-lg">
					<div
						class="mx-auto flex h-16 w-16 items-center justify-center rounded-full bg-primary/10"
					>
						<IconCloud class="size-8 text-primary" />
					</div>
					<h2 class="text-2xl font-bold">Pro Feature</h2>
					<p class="text-muted-foreground">
						Upgrade to Pro to automatically sync your processed images to Google Drive or S3
						compatible storage.
					</p>
					<Button href="/dashboard/upgrade" size="lg" class="w-full">Upgrade Now</Button>
				</div>
			</div>
		{/if}

		<div
			class="grid gap-6 md:grid-cols-2 lg:grid-cols-3 {data.isPro
				? ''
							...
							use:enhance={() => {
								return async ({ result }) => {
									if (result.type === 'success') {
										...
										toast.error(failureData?.message || 'Failed to save');
									}
								};
							}}
						>
							...
							<div class="flex gap-2">
								...
								<Button variant="ghost" size="sm" onclick={() => (showS3Form = false)}
									>Cancel</Button
								>
							</div>
						</form>
			...
			<Card class="relative">
				...
				<CardContent>
					{#if data.connectors.find((c) => c.provider === 'google_drive')}
						<div class="space-y-4">
							<div
								class="flex items-center gap-2 rounded-lg bg-green-500/10 p-3 text-xs text-green-600"
							>
					...
					{:else}
						<div class="space-y-4">
							...
							<Button
								variant="outline"
								class="w-full gap-2"
								href="/api/auth/google/login?connect=drive"
							>
